...
export const SetBudgetModal = ({ onClose, initialData }: { onClose: () => void; initialData?: Budget | null }) => {
  ...
  // only allow expense categories for budget
  ...
  const onSubmit = async (data: BudgetForm) => {
    try {
      ...
      dispatch(fetchBudget()); // refresh budget after update
      onClose();
    } catch (error: any) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div>
        ...
        {/* disable category change in edit mode */}
      ...
      <div>
        ...
        <div className="relative">
            ...
            <input
              ...
              className="block w-full pl-9 pr-3 py-2.5 bg-slate-50 border rounded-lg text-sm outline-none transition-all"
            />
        </div>
      </div>
